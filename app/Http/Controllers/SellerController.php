...
class SellerController extends Controller
{
    ...
    public function verify_broker_otp(Request $request){
        ...
        $check_broker = AddBrokers::where(['buyer_id' => $request->seller_id, 'broker_id' => $otp_verify->id, 'user_type' => 'seller'])->first();
        ...
        if($otp == $otp_verify->otp){
            ...
            if (($diff > 0) && ($minutes <= 180)) {
                ...
                $check_broker = AddBrokers::where(['buyer_id' => $request->seller_id, 'user_type' => 'seller'])->first();
